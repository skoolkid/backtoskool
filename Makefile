...
usage:
	...
	@echo "  bts      build the Back to Skool HTML disassembly"
	@echo "  bts.asm  build the Back to Skool ASM disassembly"
...
.PHONY: bts
bts:
	utils/bts2html.py $(HTML_OPTIONS)

.PHONY: bts.asm
bts.asm:
	mkdir -p $(BUILD)/asm
	utils/bts2asm.py $(ASM_OPTIONS) > $(BUILD)/asm/bts.asm
